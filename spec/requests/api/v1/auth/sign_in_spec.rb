...
RSpec.describe 'Sign In', type: :request do
  ...
  describe 'POST /api/v1/auth/sessions' do
    it 'logs in the user and returns the JSON Web Token' do
      post api_v1_auth_sessions_path, params: { email: admin.email, password: admin.password, format: :json }
    ...
    it 'does not log in the user with invalid credentials' do
      post api_v1_auth_sessions_path, params: { email: admin.email, password: 'wrong_password', format: :json }
    ...
    it 'does not log in the user with missing parameters' do
      post api_v1_auth_sessions_path, params: { email: admin.email, format: :json }
